...
f = open("seat-cases-test-"+str(NB_SEAT)+".noos",'w')

# WRITE NB_SEAT EXAMPLES IN THAT FILE

for i in range(0,NB_SEAT):
	
	# DEFINE CLASS OF SEAT
	label = random.choice(SEAT)
	legs_or_wheels = random.choice(LEGS_OR_WHEELS)
	f.write(seat_case_header(i))
	f.write(seat_case_back(label))
	f.write(seat_case_arms(label))
	f.write(seat_case_surface())
	f.write(seat_case_support())
	f.write(seat_case_legs(legs_or_wheels))
	f.write("\n")
	f.write(seat_case(i,label))
