package scripts_general_local_disagreements;
...
public class General_Local_Disagreement_Case_1 {
	...
	public static void main(String[] args) throws Exception {
		...
		/*for(Generalization ge : adam.K.getConcept("solution:bird").intensional_definition){
			System.out.println(ge.generalization().toStringNOOS(dm));
		}*/
		...
		Concept bobyChimera = new Concept(new Sign("solution:bird"), new HashSet<>(), new HashSet<>());
		Concept bobySphynx = new Concept(new Sign("solution:mammal"), new HashSet<>(), new HashSet<>());
		...
		//bobyChimera.addGeneralizations(bobyMammal.intensional_definition());
		...
		//bobySphynx.addGeneralizations(bobyBird.intensional_definition());
		
		// Add extensional definitions
		//adamChimera.addExamples(adamMammal.extensional_definition());
		...
		bobyChimera.extensional_definition.addAll(bobyBird.extensional_definition());
		...
		//bobySphynx.addExamples(bobyFish.extensional_definition());
		bobySphynx.extensional_definition.addAll(bobyMammal.extensional_definition());
		...
		adam.H.context = new HashSet<>(adamContext);
		boby.Ki.context = new HashSet<>(bobyContext);
		boby.H.context = new HashSet<>(bobyContext);
		
		// Replace K
		adam.K = new ContrastSet(new HashSet<>(), new HashSet<>(adamContext));
		boby.K = new ContrastSet(new HashSet<>(), new HashSet<>(bobyContext));
		
		for(Concept k : adam.Ki.getAllConcepts()){
			adam.K.addConcept(k.clone());
		}
		
		for(Concept k : boby.Ki.getAllConcepts()){
			boby.K.addConcept(k.clone());
		}
		...
		for(FeatureTerm f : training_set.cases){
			...
			if(adam.name(e,adam.K).size() == 1){
				 if(adam.name(e,adam.K).get(0).equals("solution:"+s))
					 score_adam ++;
			}
			
			if(boby.name(e,adam.K).size() == 1){
				 if(boby.name(e,adam.K).get(0).equals("solution:"+s))
					 score_boby ++;
			}
